...
const COMMISSION = 0.05; // Betfair commission on net winnings
...
// ── Score / minute inference ────────────────────────────────────────────────
/** Parse "2 - 0" / "2-0" correct-score runner names. Returns null for named buckets. */
function parseScoreName(name: string): { home: number; away: number } | null {
 ...
 * back price is at/below 1.15 late in the game is the current score (no more
 * goals expected by the market). Returns null when ambiguous.
 */
function inferScore(
...
function estimateMinute(kickOff: string | undefined): number {
  if (!kickOff) return 0;
...
function ouLineFromMarketType(marketType: string | undefined): number | null {
...
// ── Trade-out math (back S at B; lay at O for equal-profit hedge) ───────────
function hedgeProfit(stake: number, entryOdds: number, layOdds: number): number {
