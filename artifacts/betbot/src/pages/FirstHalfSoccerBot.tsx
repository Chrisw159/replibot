...
import {
  Timer, Power, PowerOff, Loader2, AlertCircle,
...
import {
  useGetFirstHalfSoccerConfig,
  useUpdateFirstHalfSoccerConfig,
  useStartFirstHalfSoccerBot,
  useStopFirstHalfSoccerBot,
  useGetFirstHalfSoccerStatus,
  useGetFirstHalfSoccerCandidates,
  useListFirstHalfSoccerTrades,
  useGetFirstHalfSoccerSummary,
  getGetFirstHalfSoccerConfigQueryKey,
  getGetFirstHalfSoccerStatusQueryKey,
  getGetFirstHalfSoccerCandidatesQueryKey,
  getListFirstHalfSoccerTradesQueryKey,
  getGetFirstHalfSoccerSummaryQueryKey,
...
const BOT_TITLE = "First-Half Sniper";
const BOT_DESCRIPTION = "Paper bot that rests a same-stake lay at a fixed odds offset, with a loss-capped fallback for unmatched stake.";
    ...
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-card border border-border shadow-lg rounded-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-4 overflow-y-auto space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-muted-foreground text-xs">Entry Minute (e.g. 35)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-muted-foreground text-xs">Min Goal Gap (&gt;= 2)</label>
          ...
          <div className="space-y-3 pt-2">
            <label className="flex items-center gap-2 cursor-not-allowed opacity-70">
              ...
              <span>Paper Trading Mode (Forced for 1H bot)</span>
            </label>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-border flex justify-end gap-2 bg-muted/20">
          ...
          <button onClick={() => onSave(formData)} disabled={isSaving} className="px-4 py-2 rounded-md bg-amber-500 hover:bg-amber-600 text-white text-sm font-medium transition-colors flex items-center gap-2">
...
export default function FirstHalfSoccerBot() {
  const qc = useQueryClient();
  const [isConfigOpen, setIsConfigOpen] = useState(false);
  const [actionError, setActionError] = useState<string | null>(null);

  const { data: status } = useGetFirstHalfSoccerStatus({ query: { queryKey: getGetFirstHalfSoccerStatusQueryKey(), refetchInterval: 5000 } });
  const isRunning = status?.isRunning ?? false;

  const { data: config } = useGetFirstHalfSoccerConfig();
  
  const { data: candidates, isLoading: candidatesLoading } = useGetFirstHalfSoccerCandidates(
    { query: { queryKey: getGetFirstHalfSoccerCandidatesQueryKey(), refetchInterval: isRunning ? 5000 : 30000 } },
  );
  
  const tradeParams = { limit: 50 };
  const { data: trades, isLoading: tradesLoading } = useListFirstHalfSoccerTrades(
    tradeParams,
    { query: { queryKey: getListFirstHalfSoccerTradesQueryKey(tradeParams), refetchInterval: isRunning ? 5000 : 30000 } },
  );
  
  const { data: summary } = useGetFirstHalfSoccerSummary(
    { query: { queryKey: getGetFirstHalfSoccerSummaryQueryKey(), refetchInterval: 15000 } },
  );

  const updateConfig = useUpdateFirstHalfSoccerConfig();
  const startBot = useStartFirstHalfSoccerBot();
  const stopBot = useStopFirstHalfSoccerBot();

  const handleSaveConfig = (newConfig: any) => {
    // Exclude paperMode since it's forced true on the server
    const { paperMode, id, isRunning, updatedAt, ...updateData } = newConfig;
    updateConfig.mutate({ data: updateData }, {
      onSuccess: () => {
        setIsConfigOpen(false);
        qc.invalidateQueries({ queryKey: getGetFirstHalfSoccerConfigQueryKey() });
      }
    });
  };
  ...
  const handleStart = () => {
    ...
    startBot.mutate(undefined as any, {
      onSuccess: () => qc.invalidateQueries({ queryKey: getGetFirstHalfSoccerStatusQueryKey() }),
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold tracking-tight flex items-center gap-2">
            ...
            {BOT_TITLE}
            <Badge className="bg-amber-500/15 text-amber-500 border-amber-500/30 text-xs shadow-none">Paper Mode Only</Badge>
          </h1>
          ...
          {config && (
            <p className="text-xs text-muted-foreground mt-1">
               ...
               Lay offset <span className="font-semibold text-amber-500">
        ...
        <div className="flex flex-col items-end gap-2">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 text-xs">
              ...
              />
              {isRunning
                ? <span className="text-amber-500">Running{status?.startedAt ? ` since ${new Date(status.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ""}</span>
            ...
            {isRunning ? (
              ...
              <button type="button" disabled={startBot.isPending} onClick={handleStart}
                className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-500 text-sm font-medium transition-colors">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto pr-1">
              ...
              {!candidatesLoading && (!candidates || candidates.length === 0) && (
                <div className="rounded-xl border border-dashed border-border/60 py-8 text-center text-muted-foreground text-sm flex flex-col items-center">
                  ...
                  No 1H games matching criteria right now.
              ...
              {candidates?.map(c => {
                ...
                return (
                  <div key={`${c.eventName}-${c.marketId}`} className={`p-3 rounded-lg border transition-colors ${
                    isActionable ? "border-amber-500/30 bg-amber-500/5" :
                    c.verdict === "WATCHING" ? "border-amber-500/20 bg-amber-500/5" :
                    "border-border/50 bg-card/40 opacity-80"
                  }`}>
                    <div className="flex gap-4 items-center">
                      <div className={`w-10 flex-shrink-0 text-center rounded py-1 font-bold ${isActionable ? 'bg-amber-500/20 text-amber-500' : 'bg-muted text-muted-foreground'}`}>
                        {c.minute}'
                      </div>
                      ...
                      <div className="text-right flex-shrink-0">
                        ...
                        <div className={`text-[10px] mt-1 px-1.5 py-0.5 rounded inline-block font-semibold tracking-wide ${
                          c.verdict === "ENTERED" ? "bg-amber-500/20 text-amber-500" :
                          c.verdict === "WATCHING" ? "bg-amber-500/20 text-amber-500" :
              ...
              {trades?.map(t => {
                ...
                return (
                  <div key={t.id} className={`p-3 rounded-lg border transition-colors ${
                    ...
                    isWin ? "border-amber-500/30 bg-amber-500/5" :
                    ...
                    "border-border/50 bg-card/40"
                     ...
                     <div className="flex justify-between items-end">
                       ...
                       <div className="text-right">
                         <div className={`text-base font-bold tabular-nums ${isWin ? 'text-amber-500' : isLoss ? 'text-red-400' : 'text-muted-foreground'}`}>
                    ...
                    <BarChart data={summary.dailyPnl} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
                      ...
                      <Bar dataKey="pnl" radius={[2, 2, 0, 0]} maxBarSize={40}>
                        {summary.dailyPnl.map((entry, index) => (
                          <Cell key={`cell-${index}`} fill={entry.pnl >= 0 ? 'hsl(var(--chart-2))' : 'hsl(var(--chart-4))'} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
